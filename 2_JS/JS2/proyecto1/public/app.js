...
function actualizarUI() {
  ...
  if (count < MIN) {
    label.textContent = `${count} de ${MIN} mínimos`
  } else if (count < MAX) {
    label.textContent = `✓ Mínimo alcanzado — podés agregar ${MAX - count} más`
  } else {
    label.textContent = `✓ Máximo alcanzado`
  }
...
function limpiarTodo() {
  if (!confirm(' ¿Eliminar todos los números?')) return
  numeros = []
  actualizarUI()
}
...
async function guardarEnServidor() {
  ...
  try {
    const res  = await fetch('/guardar', {
      ...
      body:    JSON.stringify({ contenido, nombre: 'numeros.txt' })
    })
...
//  DESCARGAR TXT conviertiendolo a un archivo de memoria 
